		...
		<div class="board-container">
			<div class="board-card" style="margin-top: 30px; margin-bottom: 30px;">
				<table id="main_content">
					<tr>
						<td colspan="4" class="title">후기게시판 내용보기</td>
					</tr>
					<tr>
					...
					<tr>
						<th>이름</th>
						<td><c:out value="${fv.name }"></c:out></td>
						<th>작성일(ip)</th>
						<td><fmt:formatDate value="${fv.regDate }" pattern="yyyy년 MM월 dd일(E) hh:mm:ss" /> (${fv.ip })</td>
					</tr>
					<tr>
						<th>제목</th>
						<td colspan="3"><c:out value="${fv.subject }"></c:out></td>
					</tr>
					<tr>
						<th valign="top">내용</th>
						<td colspan="3">
						...
						<td colspan="3">
							...
							<c:if test="${not empty fv.fileList }">
								<c:forEach var="fvo" items="${fv.fileList }">
									...
									<a href="${url }" title="${fvo.oriName }"><i class="axi axi-download2"></i> ${fvo.oriName }</a>
									<br />
								</c:forEach>
							</c:if>
						</td>
